...
export async function getTodayCards(limit: number = 30): Promise<Card[]> {
  ...
  const { data: allCards, error: allError } = await supabase.from("cards").select("*");
  ...
  const { data: progressCards } = await supabase.from("card_progress").select("card_id");

  const progressCardIds = new Set(
    progressCards?.map((cp: { card_id: string }) => cp.card_id) || []
  );
  const newCards =
    (allCards as Card[])?.filter((card: Card) => !progressCardIds.has(card.id)) || [];
  ...
  // Helper function to normalize card data from database
  const normalizeCard = (card: any): Card => {
    return {
      ...card,
      choices: Array.isArray(card.choices) ? card.choices : JSON.parse(card.choices || "[]"),
  ...
  // Add due cards first
  if (dueCardsData) {
    selectedCards.push(...(dueCardsData as any[]).map(normalizeCard));
  }

  // Add new cards if we need more
  if (selectedCards.length < limit) {
    const remaining = limit - selectedCards.length;
    selectedCards.push(...(newCards as any[]).slice(0, remaining).map(normalizeCard));
  }
  ...
  if (selectedCards.length < limit && allCards) {
    ...
    const remainingCards = (allCards as any[]).filter((c: any) => !selectedIds.has(c.id));
    const remaining = limit - selectedCards.length;
